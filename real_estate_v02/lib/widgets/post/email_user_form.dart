...
class EmailUserForm extends StatelessWidget {
  ...
  EmailUserForm({
    ...
    this.validate = const{
      "^([a-zA-Z0-9!#\$%^&*+\\-/_=?{|}~]+\\.?)+"
      "@[a-zA-Z0-9!#\$%^&*+\\-/_=?{|}~]+"
      "\\.([a-zA-Z0-9!#\$%^&*+\\-/_=?{|}~]{1,3}\\.?){0,2}[a-zA-Z0-9!#\$%^&*+\\-/_=?{|}~]{1,3}\$"
          : "Not a valid email!"
    },
    this.validateAuto = const {
      "^((?![.][.]).)*\$":"Double dots shouldn't be one after another",
      "^([^@.].*[^@.]|[^@.])\$":"'@' and '.' shouldn't be in first or last position",
      "^[a-zA-Z0-9!#\$%^&*+\\-/_=?{|}~.@]*\$": "Not appropriate email characters",
      "^((?!(@\\.)|(\\.@)).)*\$": "'@' and '.' should have something in between",
